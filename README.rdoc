...
devise_invitable 0.5 supports Rails 3 only and is NOT backward compatible. You can use the latest Rails 3 beta gem with the latest devise_invitable gem:
...
Configure devise_invitable inside your app (and devise if you weren't using it):
...
DeviseInvitable adds two new configuration options:
  :invite_for => It's the time a invitation is valid for. Default value is nil, which means invitation doesn't expire.
...
It uses two flash messages, :send_instructions and :updated, which are translated like other flash messages from devise.
